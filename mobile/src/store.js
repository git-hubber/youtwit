...
const networkInterface = createNetworkInterface({
  ...
  uri: 'http://10.1.1.219:3000/graphql',
});

const wsClient = new SubscriptionClient('ws://192.168.0.12:3000/subscriptions', {
  reconnect: true,
  connectionParams: {}
});
...
networkInterface.use([{
  async applyMiddleware(req, next) {
    ...
    try {
      const token = await AsyncStorage.getItem('@youtwit');
      if (token != null) {
